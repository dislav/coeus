...
> **CORS: the backend does not configure CORS.** A browser SPA on a different
> origin will fail every preflight. You have two options:
>
> 1. **Serve the frontend from the same origin** behind a reverse proxy in
>    production, and use a dev-server proxy in development (Vite/Next/webpack
>    `proxy` config) that forwards `/api` to the Coeus server.
> 2. Add CORS middleware to the backend (a server-side change — out of scope for
>    a pure frontend build, but the only way a cross-origin SPA can call the API
>    directly).
>
> Until one of these is in place, a browser frontend cannot reach the API. This
> is the single most common integration blocker.
...
```js
const API = "/api/v1";   // same-origin via proxy in dev; see §2 CORS note
...
| 409 | `duplicate` | Email already registered | "Account exists" on register |
...
1. **No CORS.** A cross-origin browser app will fail every request until you add
   a proxy or server-side CORS. See [§2](#2-conventions).
